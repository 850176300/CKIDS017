...
static inline void projectPolygon( CCPoint &axis,  Polygon &polygon, float* min, float* max)
{
 ...
 */
static inline void projectCircle(CCPoint axis, Circle circle, float* min, float* max)
{
...
bool ptInPolygon (CCPoint pt, Polygon pologon)
{
    ...
    {
        CCPoint p1 = pologon.getVectAt(i);
        CCPoint p2 = pologon.getVectAt((i + 1) % pologon.getVecNum());
        // 求解 y=p.y 与 p1p2 的交点
