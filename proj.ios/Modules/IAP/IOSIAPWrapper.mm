...
//    [alert release];
    if (ccDelegate_) {
        ...
        ccDelegate_->showDialogView([message UTF8String]);
    }
}
...
- (void)restoreFinishedWithPaymentTransitions:(NSArray*)transitions
{
    if (transitions.count == 0) {
      [self alertMessage:@"Sorry! It looks like you haven't purchased anything yet.."];
    }
}
...
- (void)restoreCompletedWithProductIdentifiers:(NSArray *)productIdentifiers
{
    if (ccDelegate_)
    {
        ...
        if([productIdentifiers count]==0){
            ...
            [self alertMessage:@"Your content has been restored!"];
        }
...
{
    // alert dialog
    [self alertMessage:@"Sorry, restore transaction failed !"];
